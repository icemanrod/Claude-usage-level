...
enum GitAnalyzer {
    ...
    static func commits(in repoPath: String, sinceDaysAgo days: Int = 30) -> [GitCommit] {
        guard let email = userEmail(in: repoPath) else {
            print("[ClaudeUsageLevel] Could not get git email for \(repoPath)")
            return []
        }
...
Run: `xcodegen generate && xcodebuild -scheme ClaudeUsageLevel -configuration Release build 2>&1 | grep -E "error:|BUILD SUCCEEDED|BUILD FAILED"`
...
```bash
xcodegen generate && xcodebuild -scheme ClaudeUsageLevel -configuration Release build
```
